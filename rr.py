...
def process_video_stream(cap):
    ...
    while True:
        ...
        for obj_id in detected_objects.keys():
        ...
        for obj_id, info in list(detected_objects.items()):
            if info['count'] < timeout:
                ...
                x1, y1, x2, y2 = predicted_box[0]  # Прогнозируемая позиция (пока можно оставить без изменений)
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)

                # Если объект не обнаруживается дольше timeout * 2, удаляем его
                if info['count'] >= timeout * 2:
